...
import { endOfDay, getDate, isWithinInterval } from "date-fns";
...
const getMidnight = (time:number):number=>{
...
export const getByHour = (offset=0) : {hour:string,count:number}[] => {
  let todayHour = Date.now()-offset*OneDay
  let endOfDay: number = getMidnight(todayHour)
  
  let tomorrow = getMidnight(todayHour + 25*OneHour)
  

  let sessionsToday : Event[] = db
  .get('events')
  .filter((event)=>{
   return  event.date >= endOfDay && event.date < tomorrow
  })
  .uniqBy('session_id')
  .value()
  const sessionsByHour : {hour:string,count:number}[] =[]

  for(let hour = tomorrow-OneHour;hour>=endOfDay;hour -= OneHour){
